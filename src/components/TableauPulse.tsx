...
// Mock useKV hook for environment compatibility
...
export function TableauPulse({ metrics }: TableauPulseProps) {
  ...
  const [activeView, setActiveView] = useState<'insights' | 'digests' | 'slack'>('insights')
  ...
  useEffect(() => {
    ...
    // Safety check for rate limiter availability
    if (llmRateLimiter) {
...
Focus on finding meaningful patterns, correlations, or anomalies that would be valuable to business leaders.`
  ...
  const getInsightIcon = (type: PulseInsight['type']) => {
    switch (type) {
      ...
      case 'trend':
        return <TrendUp weight="fill" className="text-green-500" />
      case 'anomaly':
        return <Bell weight="fill" className="text-red-500" />
      case 'achievement':
        return <CheckCircle weight="fill" className="text-green-500" />
      default:
        return <Sparkle weight="fill" className="text-blue-500" />
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <Card className="p-6 bg-gradient-to-br from-accent/10 via-card to-purple-500/10 border-accent/30">
