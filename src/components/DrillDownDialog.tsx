...
import { Download, TrendUp, TrendDown, Calendar, Tag } from '@phosphor-icons/react'
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function DrillDownDialog({ open, onOpenChange, data }: DrillDownDialogProps) {
  ...
  // Automatically select the first available view when data changes
  useEffect(() => {
    if (data) {
  ...
  // Prevent rendering if no data is available, but keep Dialog mounted if needed for transition logic
  // usually handled by the parent, but strict null check here is safe.
  if (!data) return null
          ...
          <div className="px-6 py-4 flex-1 overflow-y-auto">
                ...
                {data.metadata.highest && (
                  <Card className="p-4 bg-muted/30">
                    ...
                    <div className="truncate">
                      <p className="text-sm font-semibold mb-0.5 truncate">{data.metadata.highest.label}</p>
            ...
            <Tabs value={activeView} onValueChange={(v) => setActiveView(v as any)} className="w-full">
              <TabsList className="grid w-full grid-cols-3 mb-4">
                <TabsTrigger value="breakdown" disabled={!data.breakdown || data.breakdown.length === 0}>
                  Breakdown
                </TabsTrigger>
                <TabsTrigger value="timeline" disabled={!data.timeSeries || data.timeSeries.length === 0}>
                  Timeline
                </TabsTrigger>
